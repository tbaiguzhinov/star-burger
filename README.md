...
Рабочая версия сайта [тут](https://www.guspbru.ru/).
...
Зарегистрируйтесь на сайте [Yandex для разработчиков](https://developer.tech.yandex.ru/) и получите API ключ для сервиса "Javascript API и HTTP Геокодер". Добавьте переменную окружения `YANDEX_KEY` в файл `.env` через знак `=`.
...
Создайте файл `deploy.sh` в любом удобном для вас месте на сервере и положите туда следующий код:  
```sh
#!/bin/bash
set -e
cd ../star-burger #путь от месторасположение файла до папки star-burger
git pull
